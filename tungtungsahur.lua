...
local Blacklist, bLabel, lastUISearch, IsWaitingSus = {}, nil, 0, false
...
AvatarImage.BackgroundTransparency = 1; AvatarImage.Image = P_Serv:GetUserThumbnailAsync(LP.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
        ...
        pcall(function()
            ...
            if c > 0 then
                ...
                if LastBounty ~= -1 and c < 30000000 then
                    local d = c - LastBounty; if d > 0 and d < 50000 then TotalEarned = TotalEarned + d; SaveEarned(TotalEarned) end
                end
                LastBounty = c
            end
...
local function SyncBananaTarget()
    if bLabel and bLabel.Parent and bLabel.Text then local n = string.match(bLabel.Text, "Target %([%s]*([%w_]+)"); if n then local p = S.P:FindFirstChild(n); return p and p.Character or nil end return nil end
    if tick() - lastUISearch > 2 then lastUISearch = tick(); for _, v in ipairs(S.CG:GetDescendants()) do if v:IsA("TextLabel") and v.Text and string.find(v.Text, "Target %(") then bLabel = v; break end end if not bLabel then local pGui = LP:FindFirstChild("PlayerGui"); if pGui then for _, v in ipairs(pGui:GetDescendants()) do if v:IsA("TextLabel") and v.Text and string.find(v.Text, "Target %(") then bLabel = v; break end end end end end
    return nil
end
...
t_spawn(function()
    ...
    while t_wait(0.1) do
        ...
        pcall(function()
            ...
            local t = SyncBananaTarget(); if t then getgenv().LastTargetName = t.Name end
            ...
            if hp > 0 and hp < 4000 and not getgenv().Retreating then
                ...
                if eName then
                    ...
                    if getgenv().RetreatTracker[eName] >= 3 then
                        Blacklist[eName] = tick(); local bGuy = S.P:FindFirstChild(eName)
                        if bGuy and bGuy.Character and bGuy.Character:FindFirstChild("HumanoidRootPart") then pcall(function() bGuy.Character.HumanoidRootPart.CFrame = cf_new(0, 50000, 0) end) end
            ...
            if t and t:FindFirstChild("HumanoidRootPart") and t:FindFirstChild("Humanoid") and t.Humanoid.Health > 0 and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
                ...
                getgenv().LockedTarget = ((LP.Character.HumanoidRootPart.Position - t.HumanoidRootPart.Position).Magnitude <= 300 and not getgenv().Retreating) and t or nil
                ...
                if getgenv().LockedTarget == t then
                    ...
                    if tmr[t.Name] >= 25 then Blacklist[t.Name] = tick(); pcall(function() t.HumanoidRootPart.CFrame = cf_new(0, 50000, 0) end); getgenv().LockedTarget, tmr[t.Name] = nil, nil end
                elseif not getgenv().Retreating then tmr[t.Name] = 0 end
        ...
        if getgenv().LockedTarget and LP.Character and LP.Character:FindFirstChild("Humanoid") then
            ...
            if _G.AttackNoAnimation then
                ...
                if b and b.equipped then
                    ...
                    b.timeToNextAttack, b.hitboxMagnitude, b.active, b.blocking, b.focusStart, b.increment = 0, 60, false, false, 0, 3; b:attack()
                end
            end
        end
    end)
...
local bv = Instance.new("BodyVelocity"); bv.MaxForce = v3_new(math.huge, math.huge, math.huge); bv.Velocity = v3_new(0, 0, 0)
S.RS.Heartbeat:Connect(function()
    pcall(function()
        ...
        if t and t:FindFirstChild("HumanoidRootPart") and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if not hrp:FindFirstChild("BodyVelocity") then local n = bv:Clone(); n.Parent = hrp end
            if (hrp.Position - tHrp.Position).Magnitude > 200 then hrp.CFrame = cf_new(hrp.Position, tHrp.Position); hrp.CFrame = hrp.CFrame + hrp.CFrame.LookVector * (BaseSpeed / 10) else hrp.CFrame = tHrp.CFrame * cf_new(0, 1.5, 2.5); hrp.AssemblyLinearVelocity = v3_new(0, 0, 0); hrp.AssemblyAngularVelocity = v3_new(0, 0, 0); tHrp.AssemblyLinearVelocity = v3_new(0, 0, 0) end
            for _, v in pairs(LP.Character:GetDescendants()) do if v:IsA("BasePart") and v.CanCollide then v.CanCollide = false end end
        else if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then local c = LP.Character.HumanoidRootPart; local b = c:FindFirstChild("BodyVelocity"); if b then b:Destroy() end end end
    end)
end)
