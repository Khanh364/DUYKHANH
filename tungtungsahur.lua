    ...
    pcall(function()
        ...
        if t and t:FindFirstChild("HumanoidRootPart") and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if (hrp.Position - tHrp.Position).Magnitude > 200 then 
                hrp.CFrame = cf_new(hrp.Position, tHrp.Position); hrp.CFrame = hrp.CFrame + hrp.CFrame.LookVector * (BaseSpeed / 10) 
            else 
                hrp.CFrame = CFrame.new(predictedPos + Vector3.new(0, 2, 0), tHrp.Position)
                hrp.AssemblyLinearVelocity = v3_new(0, 0, 0); hrp.AssemblyAngularVelocity = v3_new(0, 0, 0) 
            end
            for _, v in pairs(LP.Character:GetDescendants()) do if v:IsA("BasePart") and v.CanCollide then v.CanCollide = false end end
        else 
            if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then local c = LP.Character.HumanoidRootPart; local b = c:FindFirstChild("BodyVelocity"); if b then b:Destroy() end end 
        end
    end)
end)
