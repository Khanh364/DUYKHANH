...
if not SafeGui then 
    pcall(function() 
        SafeGui = game:GetService("CoreGui") 
    end) 
end

if not SafeGui then 
    SafeGui = LP:WaitForChild("PlayerGui") 
end
...
pcall(function() 
    setclipboard("https://discord.gg/bFfNwzueg") 
end)
...
task.spawn(function()
    ...
    ps.Thickness = 2
    ps.Color = Color3.fromRGB(0, 200, 100)
    ps.Parent = Pop
    ...
    TweenService:Create(PopText, TweenInfo.new(0.4), {
        TextTransparency = 0
    }):Play()
    
    TweenService:Create(PopSub, TweenInfo.new(0.4), {
        TextTransparency = 0
    }):Play()
    ...
    TweenService:Create(PopText, TweenInfo.new(0.3), {
        TextTransparency = 1
    }):Play()
    
    TweenService:Create(PopSub, TweenInfo.new(0.3), {
        TextTransparency = 1
    }):Play()
...
local function getRealBounty()
    ...
    if ls then
        ...
        if b then 
            ...
            if valStr:match("M") then 
                return (tonumber(valStr:gsub("M", "")) or 0) * 1000000
            elseif valStr:match("K") then 
                return (tonumber(valStr:gsub("K", "")) or 0) * 1000 
            end
...
local function SaveEarned(val) 
    pcall(function() 
        if writefile then 
            writefile(EarnedFileName, tostring(val)) 
        end 
    end) 
end
...
local function LoadEarned() 
    local val = 0
    pcall(function() 
        if isfile and readfile and isfile(EarnedFileName) then 
            val = tonumber(readfile(EarnedFileName)) or 0 
        end 
    end)
    return val 
end
...
local function SaveKills(val) 
    pcall(function() 
        if writefile then 
            writefile(KillFileName, tostring(val)) 
        end 
    end) 
end
...
local function LoadKills() 
    local val = 0
    pcall(function() 
        if isfile and readfile and isfile(KillFileName) then 
            val = tonumber(readfile(KillFileName)) or 0 
        end 
    end)
    return val 
end
...
local function SaveTime(val) 
    pcall(function() 
        if writefile then 
            writefile(TimeFileName, tostring(val)) 
        end 
    end) 
end
...
local function LoadTime() 
    local val = 0
    pcall(function() 
        if isfile and readfile and isfile(TimeFileName) then 
            val = tonumber(readfile(TimeFileName)) or 0 
        end 
    end)
    return val 
end
...
AvatarImage.Position = UDim2.new(0, 10, 0, 30)
AvatarImage.BackgroundTransparency = 1
AvatarImage.Image = S.P:GetUserThumbnailAsync(LP.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
...
NameLabel.Position = UDim2.new(0, 55, 0, 37)
...
ResetBtn.Position = UDim2.new(1, -8, 1, -8)
...
DiscordBtn.Position = UDim2.new(1, -95, 1, -8)
...
local function formatNumber(n)
    local f = tostring(m_floor(math.abs(n)))
    local k = nil
    while true do 
...
local function formatTime(sec) 
    return string.format("%02d:%02d:%02d", m_floor(sec/3600), m_floor((sec%3600)/60), m_floor(sec%60)) 
end
    ...
    while t_wait(0.5) do
        ...
        if c > 0 then
            ...
            if LastBounty ~= -1 and c < 30000000 then
                ...
                if d > 0 and d < 50000 then 
                    TotalEarned = TotalEarned + d
                    SaveEarned(TotalEarned) 
                end
            end
            LastBounty = c
        ...
        pcall(function()
            local displayBounty = 0
            if c > 0 then
                displayBounty = c
            elseif LastBounty ~= -1 then
                displayBounty = LastBounty
            end
            ...
            EarnLabel.Text = "💵 EARNED: " .. plusSign .. formatNumber(TotalEarned)
            
            local tElapsed = SavedTime + (tick() - StartTime)
            TimeLabel.Text = "⏳ TIME: " .. formatTime(tElapsed)
            SaveTime(tElapsed)
            ...
            pcall(function()
                ...
                if net and net:FindFirstChild("ServerStatsItem") and net.ServerStatsItem:FindFirstChild("Data Ping") then 
                    ping = m_floor(net.ServerStatsItem["Data Ping"]:GetValue()) 
                else 
            ...
            WeaponLabel.Text = "🗡️ WEAPON: " .. toolName
            
            local raceName = "--"
            if LP:FindFirstChild("Data") and LP.Data:FindFirstChild("Race") then
                raceName = LP.Data.Race.Value
            end
            RaceLabel.Text = "🧬 RACE: " .. tostring(raceName)
            
            local teamName = "--"
            if LP.Team then
                teamName = LP.Team.Name
            end
            FactionLabel.Text = "🏴‍☠️ FACTION: " .. tostring(teamName)
        end)
    end
end)
            ...
            if obj:IsA("TextLabel") or obj:IsA("TextButton") then 
                task.delay(0.2, function() 
                    ...
                    if c >= 30000000 then 
                        ...
                        if (string.find(txt, "bounty") or string.find(txt, "honor")) and string.find(obj.Text, "%+") then 
                            local n = string.match(obj.Text:gsub(",", ""), "%d+") 
                            if n then 
                                local v = tonumber(n) 
                                if v and v > 0 and v < 50000 then 
...
local function applyAllSmoothGraphics(v)
    ...
    elseif v:IsA("Fire") or v:IsA("SpotLight") or v:IsA("Smoke") or v:IsA("Sparkles") or v:IsA("PostEffect") or v:IsA("BlurEffect") or v:IsA("BloomEffect") or v:IsA("ColorCorrectionEffect") or v:IsA("SunRaysEffect") or v:IsA("DepthOfFieldEffect") then 
        v.Enabled = false 
...
t_spawn(function() 
    pcall(function() 
        ...
        local t = S.W:FindFirstChildOfClass("Terrain") 
        if t then 
            t.WaterWaveSize = 0
            t.WaterWaveSpeed = 0
        ...
        if p ~= LP then
            ...
            if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChild("Humanoid") and char.Humanoid.Health > 0 then
                ...
                if p:FindFirstChild("Data") and p.Data:FindFirstChild("Level") then
                    tLevel = p.Data.Level.Value
                end
                
                local validLevel = (tLevel >= myLevel - 300)
                
                local isAlly = false
                if LP.Team and p.Team and LP.Team == p.Team and LP.Team.Name ~= "Pirates" then 
                    isAlly = true 
                end
                
                local notBlacklisted = true
                if Blacklist[p.Name] and tick() - Blacklist[p.Name] < 300 then
...
local function SmartEquipFruit()
    ...
    for _, v in ipairs(LP.Character:GetChildren()) do 
        if v:IsA("Tool") then 
            for _, f in ipairs(m1Priority) do 
                if v.ToolTip == "Blox Fruit" or v.Name:match(f) then 
                    return v 
                end 
            end 
        end 
...
t_spawn(function()
    ...
    while t_wait(0.1) do
                ...
                if getgenv().LockedTarget then
                    ...
                    if not trackedHums[hum] then 
                        ...
                        hum.Died:Connect(function() 
                            ...
                            SaveKills(TotalKills)
                            pcall(function() 
                                KillsLabel.Text = "⚔️ KILLS: " .. tostring(TotalKills) 
                            end) 
                        end) 
                    end
                end
        ...
        pcall(function() 
            if getgenv().LockedTarget and LP.Character and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health > 0 and not IsWaitingSus then 
...
local function GetCurrentBlade() 
    local s, r = pcall(require, LP.PlayerScripts:FindFirstChild("CombatFramework"))
    if s and r then 
        return r.activeController 
    end 
    return nil 
end
        ...
        if getgenv().LockedTarget and LP.Character and LP.Character:FindFirstChild("Humanoid") then
            ...
            if _G.AttackNoAnimation then
                ...
                if b and b.equipped then
                    if b.animator and b.animator.ani then 
                        b.animator.ani:Stop() 
                    end
                    b.timeToNextAttack = 0
                    b.hitboxMagnitude = 80
                    b.active = false
                    b.blocking = false
                    b.focusStart = 0
                    b.increment = 3
...
task.spawn(function()
    while task.wait() do
        if _G.AttackNoAnimation and getgenv().LockedTarget then
            pcall(function() 
                local b = GetCurrentBlade()
                if b and b.equipped and b.currentWeaponModel and RigEvent then 
                    RigEvent:FireServer("hit", b.hitboxMagnitude, b.currentWeaponModel, b.currentWeaponModel.CFrame) 
                end 
            end)
        end
    end
...
local function applyUnderwater(char)
    ...
    if hrp then 
        hrp.CustomPhysicalProperties = PhysicalProperties.new(0.7, 0.3, 0.5, 0.1, 0.1) 
    end
end

if LP.Character then 
    t_spawn(function() 
        applyUnderwater(LP.Character) 
    end) 
end
...
local function preventSitting(char) 
    local h = char:WaitForChild("Humanoid", 5)
    if h then 
