...
S.RS.Heartbeat:Connect(function(dt)
    pcall(function()
        ...
        if t and t:FindFirstChild("HumanoidRootPart") and char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if dist > 15 then
                local moveDist = math.clamp(safeSpeed * dt, 0, dist)
                hrp.CFrame = CFrame.lookAt(hrp.Position, tHrp.Position) * CFrame.new(0, 0, -moveDist)
            else
                hrp.CFrame = CFrame.new(predictedPos + Vector3.new(0, 3, 0), tHrp.Position)
            end
        end
    end)
end)
...
local function SmartServerHop()
    pcall(function()
        local req = S.HTTP:JSONDecode(game:HttpGet("https://games.roproxy.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Desc&limit=100"))
        local srv = {}
        if req and req.data then 
            for _, v in ipairs(req.data) do 
                if type(v) == "table" and v.playing and v.maxPlayers and v.playing < (v.maxPlayers - 2) and v.id ~= game.JobId then 
                    table.insert(srv, v.id) 
                end 
            end 
        end
        if #srv > 0 then 
            S.TS:TeleportToPlaceInstance(game.PlaceId, srv[m_random(1, #srv)], LP) 
        end
    end)
end
