...
AvatarImage.Position = UDim2.new(0.5, -18, 0, 14)
...
NameLabel.Position = UDim2.new(0, 0, 0, 54)
...
local function CreateLabel(obj, pos, size, color, text, textSize)
    ...
    obj.TextSize = textSize or 10
    obj.Text = text
    obj.TextXAlignment = Enum.TextXAlignment.Center
end

CreateLabel(Title, UDim2.new(0, 0, 0, 4), UDim2.new(1, 0, 0, 12), Color3.fromRGB(85, 170, 255), "👑 Skibidi VIP Pro", 11)

CreateLabel(TotalLabel,   UDim2.new(0, 4, 0, 76),  UDim2.new(0.5, -6, 0, 16), Color3.fromRGB(255, 255, 255), "💰 TOTAL: --")
CreateLabel(EarnLabel,    UDim2.new(0, 4, 0, 96),  UDim2.new(0.5, -6, 0, 16), Color3.fromRGB(0, 255, 120),   "💵 EARNED: 0")
CreateLabel(KillsLabel,   UDim2.new(0, 4, 0, 116), UDim2.new(0.5, -6, 0, 16), Color3.fromRGB(255, 100, 100), "⚔️ KILLS: " .. tostring(TotalKills))
CreateLabel(TimeLabel,    UDim2.new(0, 4, 0, 136), UDim2.new(0.5, -6, 0, 16), Color3.fromRGB(255, 200, 50),  "⏳ TIME: 00:00")

CreateLabel(WeaponLabel,  UDim2.new(0.5, 2, 0, 76),  UDim2.new(0.5, -6, 0, 16), Color3.fromRGB(255, 170, 0),  "🗡️ WEAPON: --")
CreateLabel(RaceLabel,    UDim2.new(0.5, 2, 0, 96),  UDim2.new(0.5, -6, 0, 16), Color3.fromRGB(170, 85, 255), "🧬 RACE: --")
CreateLabel(FactionLabel, UDim2.new(0.5, 2, 0, 116), UDim2.new(0.5, -6, 0, 16), Color3.fromRGB(85, 255, 127), "🏴‍☠️ TEAM: --")

local Divider = Instance.new("Frame")
Divider.Parent = MainFrame
Divider.Size = UDim2.new(0.88, 0, 0, 1)
Divider.Position = UDim2.new(0.06, 0, 0, 72)
Divider.BackgroundColor3 = Color3.fromRGB(85, 170, 255)
Divider.BorderSizePixel = 0

local Divider2 = Instance.new("Frame")
Divider2.Parent = MainFrame
Divider2.Size = UDim2.new(0.88, 0, 0, 1)
Divider2.Position = UDim2.new(0.06, 0, 0, 158)
Divider2.BackgroundColor3 = Color3.fromRGB(85, 170, 255)
Divider2.BorderSizePixel = 0

CreateLabel(StatLabel, UDim2.new(0, 0, 0, 162), UDim2.new(1, 0, 0, 14), Color3.fromRGB(200, 200, 200), "⚡ FPS: -- | 📶 PING: --", 9)

ResetBtn.Parent = MainFrame
ResetBtn.Size = UDim2.new(0, 80, 0, 22)
ResetBtn.AnchorPoint = Vector2.new(0.5, 0)
ResetBtn.Position = UDim2.new(0.5, 0, 0, 180)
...
DiscordBtn.Size = UDim2.new(0, 80, 0, 22)
DiscordBtn.AnchorPoint = Vector2.new(0.5, 0)
DiscordBtn.Position = UDim2.new(0.5, -44, 0, 180)
...
t_spawn(function()
    pcall(function()
        if not getgenv().Setting.DeleteMap then return end

        local pendingChars = {}

        local function registerChar(char)
            pendingChars[char] = true
            task.delay(10, function() pendingChars[char] = nil end)
        end

        for _, plr in ipairs(S.P:GetPlayers()) do
        ...
        LP.CharacterAdded:Connect(registerChar)

        local function isPlayerCharacter(obj)
            if pendingChars[obj] then return true end
            for _, plr in ipairs(S.P:GetPlayers()) do
                if plr.Character and obj == plr.Character then return true end
            end
            return false
        end
        ...
        local function shouldKeep(obj)
            ...
            if obj:IsA("Terrain") or obj:IsA("Camera") then return true end
            if isPlayerCharacter(obj) then return true end
            return false
        end

        for _, obj in ipairs(S.W:GetChildren()) do
            pcall(function()
                if not shouldKeep(obj) then obj:Destroy() end
            end)
        end

        S.W.ChildAdded:Connect(function(obj)
            task.wait(0.15)
            pcall(function()
                if not shouldKeep(obj) then obj:Destroy() end
            end)
    ...
    while t_wait(0.1) do
        ...
        pcall(function()
            if getgenv().LockedTarget and LP.Character and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health > 0 then
                if not IsWaitingSus and getgenv().Setting.AutoEquipFruit then
...
local function SuperFastM1()
    if not getgenv().Setting.AutoClick then return end
    pcall(function()
        if getgenv().LockedTarget and LP.Character and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health > 0 then
...
RS.RenderStepped:Connect(SuperFastM1)
RS.Heartbeat:Connect(SuperFastM1)

t_spawn(function()
    RS.Stepped:Connect(function()
        pcall(function()
            if getgenv().Setting.AttackNoAnimation and getgenv().LockedTarget and LP.Character then
...
RS.Heartbeat:Connect(function()
    pcall(function()
        ...
        if t and t:FindFirstChild("HumanoidRootPart") and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local targetCFrame = tHrp.CFrame * CFrame.new(0, 1.5, 2.5) 
