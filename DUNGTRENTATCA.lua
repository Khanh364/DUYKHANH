...
task.spawn(function()
    RS.Heartbeat:Connect(function()
        if _G.AttackNoAnimation and getgenv().LockedTarget then
            pcall(function()
                local blade = GetCurrentBlade()
                if blade and blade.equipped and blade.currentWeaponModel and RigEvent then
                    local tHrp = getgenv().LockedTarget:FindFirstChild("HumanoidRootPart")
                    local hrp = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
                    if tHrp and hrp and (tHrp.Position - hrp.Position).Magnitude < 30 then
                        RigEvent:FireServer("hit", blade.hitboxMagnitude, blade.currentWeaponModel, tHrp.CFrame)
                    end
                end
            end)
        end
    ...
    pcall(function()
        ...
        if t and t:FindFirstChild("HumanoidRootPart") and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if d > 150 then
                ...
                local offset = tHrp.CFrame.LookVector * -1.5
...
t_spawn(function()
    while t_wait(0.5) do
        pcall(function()
            local t = getgenv().LockedTarget
            local hrp = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
            if getgenv().Setting.SpamDash and t and hrp and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health > 0 then
