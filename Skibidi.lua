repeat task.wait() until game:IsLoaded()

local TweenService = game:GetService("TweenService")
local P_Serv = game:GetService("Players")
local LP = P_Serv.LocalPlayer or P_Serv:GetPropertyChangedSignal("LocalPlayer"):Wait()

local SafeGui
pcall(function() if type(gethui) == "function" then SafeGui = gethui() end end)
if not SafeGui then pcall(function() SafeGui = game:GetService("CoreGui") end) end
if not SafeGui then SafeGui = LP:WaitForChild("PlayerGui") end

local LoadGui = Instance.new("ScreenGui")
LoadGui.Name = "SkibidiLoadingUI_" .. tostring(math.random(1000,9999))
LoadGui.ResetOnSpawn = false
LoadGui.Parent = SafeGui

local LoadFrame = Instance.new("Frame")
LoadFrame.Size = UDim2.new(0, 280, 0, 140)
LoadFrame.Position = UDim2.new(0.5, -140, 0.5, -70)
LoadFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
LoadFrame.BorderSizePixel = 0
LoadFrame.Parent = LoadGui
Instance.new("UICorner", LoadFrame).CornerRadius = UDim.new(0, 10)
local ls = Instance.new("UIStroke", LoadFrame)
ls.Thickness = 1.5
ls.Color = Color3.fromRGB(85, 170, 255)

local WelcomeLabel = Instance.new("TextLabel")
WelcomeLabel.Size = UDim2.new(1, 0, 0, 20)
WelcomeLabel.Position = UDim2.new(0, 0, 0, 10)
WelcomeLabel.BackgroundTransparency = 1
WelcomeLabel.Text = "👋 Xin chào, " .. LP.Name .. "!"
WelcomeLabel.TextColor3 = Color3.fromRGB(255, 220, 80)
WelcomeLabel.Font = Enum.Font.GothamBold
WelcomeLabel.TextSize = 13
WelcomeLabel.Parent = LoadFrame

local LoadTitle = Instance.new("TextLabel")
LoadTitle.Size = UDim2.new(1, 0, 0, 24)
LoadTitle.Position = UDim2.new(0, 0, 0, 32)
LoadTitle.BackgroundTransparency = 1
LoadTitle.Text = "Skibidi VIP Pro"
LoadTitle.TextColor3 = Color3.fromRGB(85, 170, 255)
LoadTitle.Font = Enum.Font.GothamBold
LoadTitle.TextSize = 15
LoadTitle.Parent = LoadFrame

local SubLabel = Instance.new("TextLabel")
SubLabel.Size = UDim2.new(1, 0, 0, 18)
SubLabel.Position = UDim2.new(0, 0, 0, 58)
SubLabel.BackgroundTransparency = 1
SubLabel.Text = "discord.gg/bFfNwzueg"
SubLabel.TextColor3 = Color3.fromRGB(88, 101, 242)
SubLabel.Font = Enum.Font.Gotham
SubLabel.TextSize = 12
SubLabel.Parent = LoadFrame

local ProgressBg = Instance.new("Frame")
ProgressBg.Size = UDim2.new(0.82, 0, 0, 8)
ProgressBg.Position = UDim2.new(0.09, 0, 0, 88)
ProgressBg.BackgroundColor3 = Color3.fromRGB(35, 35, 45)
ProgressBg.BorderSizePixel = 0
ProgressBg.Parent = LoadFrame
Instance.new("UICorner", ProgressBg).CornerRadius = UDim.new(0, 4)

local ProgressBar = Instance.new("Frame")
ProgressBar.Size = UDim2.new(0, 0, 1, 0)
ProgressBar.BackgroundColor3 = Color3.fromRGB(85, 170, 255)
ProgressBar.BorderSizePixel = 0
ProgressBar.Parent = ProgressBg
Instance.new("UICorner", ProgressBar).CornerRadius = UDim.new(0, 4)

local StatusLabel = Instance.new("TextLabel")
StatusLabel.Size = UDim2.new(1, 0, 0, 18)
StatusLabel.Position = UDim2.new(0, 0, 0, 110)
StatusLabel.BackgroundTransparency = 1
StatusLabel.Text = "Loading..."
StatusLabel.TextColor3 = Color3.fromRGB(140, 140, 160)
StatusLabel.Font = Enum.Font.Gotham
StatusLabel.TextSize = 11
StatusLabel.Parent = LoadFrame

TweenService:Create(ProgressBar, TweenInfo.new(1.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, 0)}):Play()
task.wait(1.5)

StatusLabel.Text = "✅ Loaded!"
StatusLabel.TextColor3 = Color3.fromRGB(0, 220, 110)
pcall(function() setclipboard("https://discord.gg/bFfNwzueg") end)
SubLabel.Text = "✅ Đã sao chép Server Discord!"
SubLabel.TextColor3 = Color3.fromRGB(0, 220, 110)
task.wait(0.2)

LoadGui:Destroy()
task.spawn(function()
    local PopGui = Instance.new("ScreenGui")
    PopGui.Name = "WelcomePopup_" .. tostring(math.random(1000,9999))
    PopGui.ResetOnSpawn = false
    PopGui.Parent = SafeGui

    local Pop = Instance.new("Frame")
    Pop.Size = UDim2.new(0, 340, 0, 62)
    Pop.Position = UDim2.new(0.5, -170, 0, -70)
    Pop.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
    Pop.BackgroundTransparency = 1
    Pop.BorderSizePixel = 0
    Pop.Parent = PopGui
    Instance.new("UICorner", Pop).CornerRadius = UDim.new(0, 8)
    local ps = Instance.new("UIStroke", Pop)
    ps.Thickness = 2; ps.Color = Color3.fromRGB(85, 170, 255)

    local PopText = Instance.new("TextLabel")
    PopText.Size = UDim2.new(1, 0, 0.52, 0)
    PopText.Position = UDim2.new(0, 0, 0.04, 0)
    PopText.BackgroundTransparency = 1
    PopText.Text = "👋 Chào mừng, " .. LP.Name .. "!"
    PopText.TextColor3 = Color3.fromRGB(85, 170, 255)
    PopText.Font = Enum.Font.GothamBold
    PopText.TextSize = 15
    PopText.TextTransparency = 1
    PopText.Parent = Pop

    local PopSub = Instance.new("TextLabel")
    PopSub.Size = UDim2.new(1, 0, 0.38, 0)
    PopSub.Position = UDim2.new(0, 0, 0.58, 0)
    PopSub.BackgroundTransparency = 1
    PopSub.Text = "✅ Skibidi VIP Pro đã sẵn sàng!"
    PopSub.TextColor3 = Color3.fromRGB(0, 255, 120)
    PopSub.Font = Enum.Font.GothamBold
    PopSub.TextSize = 12
    PopSub.TextTransparency = 1
    PopSub.Parent = Pop

    TweenService:Create(Pop, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Position = UDim2.new(0.5, -170, 0, -55),
        BackgroundTransparency = 0
    }):Play()
    TweenService:Create(PopText, TweenInfo.new(0.4), {TextTransparency = 0}):Play()
    TweenService:Create(PopSub, TweenInfo.new(0.4), {TextTransparency = 0}):Play()

    task.wait(2.5)

    TweenService:Create(Pop, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
        Position = UDim2.new(0.5, -170, 0, -70),
        BackgroundTransparency = 1
    }):Play()
    TweenService:Create(PopText, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
    TweenService:Create(PopSub, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
    task.wait(0.35)
    PopGui:Destroy()
end)

local S = {
    P = game:GetService("Players"), W = game:GetService("Workspace"),
    RS = game:GetService("RunService"), V = game:GetService("VirtualInputManager"),
    L = game:GetService("Lighting"), ST = game:GetService("Stats"),
    GS = game:GetService("GuiService"), TS = game:GetService("TeleportService"),
    HTTP = game:GetService("HttpService"), CG = game:GetService("CoreGui")
}

local t_wait, t_spawn = task.wait, task.spawn
local m_floor = math.floor
local v3_new, cf_new = Vector3.new, CFrame.new

getgenv().Setting = getgenv().Setting or {
    Hitbox = { Enabled = true, Size = 60, Transparency = 0.7 },
    DeleteMap = true,
    AntiSusKill = true,
    SpamDash = true,
    AutoHaki = true,
    AutoAwakening = true,
    AutoSkills = true,
    HideOthers = true,
    TargetRadius = 15000 
}

getgenv().LockedTarget = nil
getgenv().Retreating = false
getgenv().RetreatTracker = getgenv().RetreatTracker or {}
getgenv().LastTargetName = nil
local Blacklist = {}
local IsWaitingSus = false

local function getRealBounty()
    local ls = LP:FindFirstChild("leaderstats")
    if ls then
        local b = ls:FindFirstChild("Bounty/Honor") or ls:FindFirstChild("Bounty")
        if b then 
            local val = tostring(b.Value):upper()
            val = val:gsub(",", ""):gsub("%$", "")
            if val:match("M") then
                return (tonumber(val:gsub("M", "")) or 0) * 1000000
            elseif val:match("K") then
                return (tonumber(val:gsub("K", "")) or 0) * 1000
            end
            return tonumber(val) or 0
        end
    end
    return 0
end

local EarnedFileName = "SkibidiEarned_" .. LP.UserId .. ".txt"
local function SaveEarned(val) pcall(function() if writefile then writefile(EarnedFileName, tostring(val)) end end) end
local function LoadEarned()
    local val = 0
    pcall(function() if isfile and readfile and isfile(EarnedFileName) then val = tonumber(readfile(EarnedFileName)) or 0 end end)
    return val
end

local KillFileName = "SkibidiKills_" .. LP.UserId .. ".txt"
local function SaveKills(val) pcall(function() if writefile then writefile(KillFileName, tostring(val)) end end) end
local function LoadKills()
    local val = 0
    pcall(function() if isfile and readfile and isfile(KillFileName) then val = tonumber(readfile(KillFileName)) or 0 end end)
    return val
end

local TimeFileName = "SkibidiTime_" .. LP.UserId .. ".txt"
local function SaveTime(val) pcall(function() if writefile then writefile(TimeFileName, tostring(val)) end end) end
local function LoadTime()
    local val = 0
    pcall(function() if isfile and readfile and isfile(TimeFileName) then val = tonumber(readfile(TimeFileName)) or 0 end end)
    return val
end

local TotalEarned = LoadEarned() 
local LastBounty = -1
local TotalKills = LoadKills()
local SavedTime = LoadTime()
local StartTime = tick()
local CurrentFPS = 0
local FrameCount = 0
local LastFPSTick = tick()

local ScreenGui = Instance.new("ScreenGui")
local MainFrame = Instance.new("Frame")
local UIStroke = Instance.new("UIStroke")
local UICorner = Instance.new("UICorner")
local Title = Instance.new("TextLabel")

local AvatarImage = Instance.new("ImageLabel")
local AvatarCorner = Instance.new("UICorner")
local NameLabel = Instance.new("TextLabel")

local TotalLabel = Instance.new("TextLabel")
local EarnLabel = Instance.new("TextLabel")
local KillsLabel = Instance.new("TextLabel") 
local TimeLabel = Instance.new("TextLabel")
local WeaponLabel = Instance.new("TextLabel")
local RaceLabel = Instance.new("TextLabel")
local FactionLabel = Instance.new("TextLabel")
local StatLabel = Instance.new("TextLabel")
local ResetBtn = Instance.new("TextButton")
local ResetCorner = Instance.new("UICorner")
local DiscordBtn = Instance.new("TextButton")
local DiscordCorner = Instance.new("UICorner")

local ToggleBtn = Instance.new("TextButton")
local ToggleCorner = Instance.new("UICorner")
local ToggleStroke = Instance.new("UIStroke")

ScreenGui.Name = "SkibidiProUI_" .. tostring(math.random(1000,9999))
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = SafeGui

MainFrame.Parent = ScreenGui
MainFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
MainFrame.Size = UDim2.new(0, 480, 0, 140) 
MainFrame.Active = true
MainFrame.Draggable = true 

UICorner.CornerRadius = UDim.new(0, 8)
UICorner.Parent = MainFrame
UIStroke.Parent = MainFrame
UIStroke.Thickness = 2
UIStroke.Color = Color3.fromRGB(85, 170, 255)
UIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

AvatarImage.Parent = MainFrame
AvatarImage.Size = UDim2.new(0, 40, 0, 40)
AvatarImage.Position = UDim2.new(0, 15, 0, 35)
AvatarImage.BackgroundTransparency = 1
AvatarImage.Image = game:GetService("Players"):GetUserThumbnailAsync(LP.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
AvatarCorner.CornerRadius = UDim.new(1, 0)
AvatarCorner.Parent = AvatarImage

NameLabel.Parent = MainFrame
NameLabel.Size = UDim2.new(0, 100, 0, 20)
NameLabel.Position = UDim2.new(0, 60, 0, 45)
NameLabel.BackgroundTransparency = 1
NameLabel.Font = Enum.Font.GothamBold
NameLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
NameLabel.TextSize = 13
NameLabel.TextXAlignment = Enum.TextXAlignment.Left
NameLabel.Text = LP.DisplayName

local function CreateLabel(obj, pos, size, color, text, textSize)
    obj.Parent = MainFrame
    obj.BackgroundTransparency = 1
    obj.Position = pos
    obj.Size = size
    obj.Font = Enum.Font.GothamBold
    obj.TextColor3 = color
    obj.TextSize = textSize or 12
    obj.Text = text
    obj.TextXAlignment = Enum.TextXAlignment.Left
end

CreateLabel(Title, UDim2.new(0, 15, 0, 10), UDim2.new(0, 200, 0, 20), Color3.fromRGB(85, 170, 255), "👑 Skibidi VIP Pro by khánh", 14)
CreateLabel(TotalLabel, UDim2.new(0, 170, 0, 35), UDim2.new(0, 140, 0, 20), Color3.fromRGB(255, 255, 255), "💰 TOTAL: --")
CreateLabel(EarnLabel, UDim2.new(0, 170, 0, 55), UDim2.new(0, 140, 0, 20), Color3.fromRGB(0, 255, 120), "💵 EARNED: 0")
CreateLabel(KillsLabel, UDim2.new(0, 170, 0, 75), UDim2.new(0, 140, 0, 20), Color3.fromRGB(255, 100, 100), "⚔️ KILLS: " .. tostring(TotalKills)) 
CreateLabel(TimeLabel, UDim2.new(0, 170, 0, 95), UDim2.new(0, 140, 0, 20), Color3.fromRGB(255, 200, 50), "⏳ TIME: 00:00:00")
CreateLabel(WeaponLabel, UDim2.new(0, 310, 0, 35), UDim2.new(0, 160, 0, 20), Color3.fromRGB(255, 170, 0), "🗡️ WEAPON: None")
CreateLabel(RaceLabel, UDim2.new(0, 310, 0, 55), UDim2.new(0, 160, 0, 20), Color3.fromRGB(170, 85, 255), "🧬 RACE: --")
CreateLabel(FactionLabel, UDim2.new(0, 310, 0, 75), UDim2.new(0, 160, 0, 20), Color3.fromRGB(85, 255, 127), "🏴‍☠️ FACTION: --")
CreateLabel(StatLabel, UDim2.new(0, 15, 1, -10), UDim2.new(0, 200, 0, 20), Color3.fromRGB(200, 200, 200), "⚡ FPS: -- |\n📶 PING: -- ms", 11)
StatLabel.AnchorPoint = Vector2.new(0, 1)

ResetBtn.Parent = MainFrame
ResetBtn.Size = UDim2.new(0, 90, 0, 25)
ResetBtn.AnchorPoint = Vector2.new(1, 1)
ResetBtn.Position = UDim2.new(1, -10, 1, -10)
ResetBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
ResetBtn.Text = "Reset"
ResetBtn.TextColor3 = Color3.new(1,1,1)
ResetBtn.Font = Enum.Font.GothamBold
ResetBtn.TextSize = 11
ResetCorner.CornerRadius = UDim.new(0, 6)
ResetCorner.Parent = ResetBtn

DiscordBtn.Parent = MainFrame
DiscordBtn.Size = UDim2.new(0, 90, 0, 25)
DiscordBtn.AnchorPoint = Vector2.new(1, 1)
DiscordBtn.Position = UDim2.new(1, -110, 1, -10)
DiscordBtn.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
DiscordBtn.Text = "Discord"
DiscordBtn.TextColor3 = Color3.new(1, 1, 1)
DiscordBtn.Font = Enum.Font.GothamBold
DiscordBtn.TextSize = 11
DiscordCorner.CornerRadius = UDim.new(0, 6)
DiscordCorner.Parent = DiscordBtn

DiscordBtn.MouseButton1Click:Connect(function()
    pcall(function() setclipboard("https://discord.gg/bFfNwzueg") end)
    local oldText = DiscordBtn.Text
    local oldColor = DiscordBtn.BackgroundColor3
    DiscordBtn.Text = "Copied!"
    DiscordBtn.BackgroundColor3 = Color3.fromRGB(0, 200, 100)
    task.wait(1.5)
    DiscordBtn.Text = oldText
    DiscordBtn.BackgroundColor3 = oldColor
end)

ToggleBtn.Parent = ScreenGui
ToggleBtn.Size = UDim2.new(0, 35, 0, 35)
ToggleBtn.Position = UDim2.new(0.05, 0, 0.95, -160)
ToggleBtn.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
ToggleBtn.Text = "Khanh"
ToggleBtn.TextColor3 = Color3.fromRGB(85, 170, 255)
ToggleBtn.Font = Enum.Font.GothamBold
ToggleBtn.TextSize = 12
ToggleBtn.Active = true
ToggleBtn.Draggable = true
ToggleCorner.CornerRadius = UDim.new(0, 8)
ToggleCorner.Parent = ToggleBtn
ToggleStroke.Parent = ToggleBtn
ToggleStroke.Thickness = 2
ToggleStroke.Color = Color3.fromRGB(85, 170, 255)
ToggleStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

ToggleBtn.MouseButton1Click:Connect(function()
    MainFrame.Visible = not MainFrame.Visible
end)
local function formatNumber(n)
    local formatted = tostring(math.floor(math.abs(n)))
    local k
    while true do
        formatted, k = string.gsub(formatted, "^(-?%d+)(%d%d%d)", '%1,%2')
        if (k==0) then break end
    end
    return (n < 0 and "-" or "") .. formatted
end

local function formatTime(seconds)
    local h = math.floor(seconds / 3600)
    local m = math.floor((seconds % 3600) / 60)
    local s = math.floor(seconds % 60)
    return string.format("%02d:%02d:%02d", h, m, s)
end

ResetBtn.MouseButton1Click:Connect(function()
    TotalEarned = 0
    LastBounty = getRealBounty()
    SaveEarned(TotalEarned)
    TotalKills = 0
    SaveKills(TotalKills)
    SavedTime = 0
    SaveTime(0)
    StartTime = tick()
    EarnLabel.Text = "💵 EARNED: 0"
    KillsLabel.Text = "⚔️ KILLS: 0"
    TimeLabel.Text = "⏳ TIME: 00:00:00"
end)

S.RS.RenderStepped:Connect(function()
    FrameCount = FrameCount + 1
    local now = tick()
    if now - LastFPSTick >= 1 then
        CurrentFPS = FrameCount
        FrameCount = 0
        LastFPSTick = now
    end
end)

t_spawn(function()
    local ls = LP:WaitForChild("leaderstats", 10)
    local bountyObj = ls and (ls:FindFirstChild("Bounty/Honor") or ls:FindFirstChild("Bounty"))
    
    local function FastUpdateBounty()
        pcall(function()
            local current = getRealBounty()
            if current > 0 then
                if LastBounty == -1 then LastBounty = current end
                if LastBounty ~= -1 then
                    local diff = current - LastBounty
                    if diff > 0 and diff < 100000 then 
                        TotalEarned = TotalEarned + diff
                        SaveEarned(TotalEarned)
                    end
                end
                LastBounty = current
            end
            local displayBounty = (current > 0) and current or (LastBounty ~= -1 and LastBounty or 0)
            TotalLabel.Text = "💰 TOTAL: " .. formatNumber(displayBounty)
            EarnLabel.Text = "💵 EARNED: " .. (TotalEarned > 0 and "+" or "") .. formatNumber(TotalEarned)
        end)
    end

    if bountyObj then
        bountyObj:GetPropertyChangedSignal("Value"):Connect(FastUpdateBounty)
    end

    while t_wait(0.5) do
        pcall(function()
            FastUpdateBounty() 
            local currentSessionTime = tick() - StartTime
            local totalElapsedTime = SavedTime + currentSessionTime
            TimeLabel.Text = "⏳ TIME: " .. formatTime(totalElapsedTime)
            SaveTime(totalElapsedTime)
            
            local ping = 0
            pcall(function() ping = m_floor(LP:GetNetworkPing() * 1000) end)
            StatLabel.Text = string.format("⚡ FPS: %d |\n📶 PING: %d ms", CurrentFPS, ping)

            local tool = LP.Character and LP.Character:FindFirstChildOfClass("Tool")
            WeaponLabel.Text = "🗡️ WEAPON: " .. (tool and tool.Name or "None")

            local race = "--"
            if LP:FindFirstChild("Data") and LP.Data:FindFirstChild("Race") then
                race = LP.Data.Race.Value
            end
            RaceLabel.Text = "🧬 RACE: " .. tostring(race)

            local faction = "--"
            if LP.Team then faction = LP.Team.Name end
            FactionLabel.Text = "🏴‍☠️ FACTION: " .. tostring(faction)
        end)
    end
end)

t_spawn(function()
    pcall(function()
        local processedTexts = {}
        local function checkText(obj)
            pcall(function()
                local txt = obj.Text or ""
                if txt == "" then return end
                local txtLower = string.lower(txt)
                
                if (string.find(txtLower, "defeated") or string.find(txtLower, "killed")) and not processedTexts[txt] then
                    processedTexts[txt] = true
                    TotalKills = TotalKills + 1
                    SaveKills(TotalKills)
                    if KillsLabel then KillsLabel.Text = "⚔️ KILLS: " .. tostring(TotalKills) end
                    task.delay(5, function() processedTexts[txt] = nil end)
                end
            end)
        end

        local pGui = LP:WaitForChild("PlayerGui", 10)
        if pGui then
            for _, v in pairs(pGui:GetDescendants()) do
                if v:IsA("TextLabel") or v:IsA("TextButton") then
                    v:GetPropertyChangedSignal("Text"):Connect(function() checkText(v) end)
                    checkText(v)
                end
            end
            pGui.DescendantAdded:Connect(function(obj)
                if obj:IsA("TextLabel") or obj:IsA("TextButton") then
                    obj:GetPropertyChangedSignal("Text"):Connect(function() checkText(obj) end)
                    task.delay(0.1, function() checkText(obj) end)
                end
            end)
        end
    end)
end)

local function FindNearestPlayer()
    local nearest = nil
    local minDist = getgenv().Setting.TargetRadius or 15000
    pcall(function()
        local myPos = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") and LP.Character.HumanoidRootPart.Position
        if not myPos then return end

        for _, player in ipairs(S.P:GetPlayers()) do
            if player ~= LP and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") then
                if player.Character.Humanoid.Health > 0 and (not Blacklist[player.Name] or tick() - Blacklist[player.Name] > 300) then
                    
                    local isSafe = false
                    if player.Character:FindFirstChildOfClass("ForceField") then
                        isSafe = true
                    end
                    pcall(function()
                        if player:FindFirstChild("Data") and player.Data:FindFirstChild("PvP") then
                            if player.Data.PvP.Value == 0 or player.Data.PvP.Value == false then
                                isSafe = true
                            end
                        end
                    end)

                    if not isSafe then
                        local dist = (player.Character.HumanoidRootPart.Position - myPos).Magnitude
                        if dist < minDist then
                            minDist = dist
                            nearest = player.Character
                        end
                    end
                end
            end
        end
    end)
    return nearest
end

t_spawn(function()
    local trackedHumanoids = {}
    while t_wait(0.1) do
        pcall(function()
            local hp = LP.Character and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health or 0
            
            if hp >= 7000 and getgenv().Retreating then getgenv().Retreating = false end
            
            if hp > 0 and hp < 4000 and not getgenv().Retreating then
                getgenv().Retreating = true
                if getgenv().LastTargetName then
                    getgenv().RetreatTracker[getgenv().LastTargetName] = (getgenv().RetreatTracker[getgenv().LastTargetName] or 0) + 1
                    if getgenv().RetreatTracker[getgenv().LastTargetName] >= 3 then
                        Blacklist[getgenv().LastTargetName] = tick()
                        getgenv().LockedTarget = nil
                        return
                    end
                end
            end

            if not getgenv().Retreating then
                local target = FindNearestPlayer()
                getgenv().LockedTarget = target
                
                if target then
                    getgenv().LastTargetName = target.Name
                    local eHum = target:FindFirstChild("Humanoid")
                    if eHum and not trackedHumanoids[eHum] then
                        trackedHumanoids[eHum] = true
                        eHum.Died:Connect(function()
                            if getgenv().LockedTarget == target or getgenv().LastTargetName == target.Name then
                                TotalKills = TotalKills + 1
                                SaveKills(TotalKills)
                                if KillsLabel then KillsLabel.Text = "⚔️ KILLS: " .. tostring(TotalKills) end
                            end
                        end)
                    end
                    
                    if getgenv().Setting.Hitbox and getgenv().Setting.Hitbox.Enabled and target:FindFirstChild("HumanoidRootPart") then
                        local hrp = target.HumanoidRootPart
                        local sz = getgenv().Setting.Hitbox.Size or 60
                        if hrp.Size.X ~= sz then
                            hrp.Size = v3_new(sz, sz, sz)
                            hrp.Transparency = getgenv().Setting.Hitbox.Transparency or 0.7
                            hrp.CanCollide = false
                        end
                    end
                end
            else
                getgenv().LockedTarget = nil
            end
        end)
    end
end)

local BodyVelocity = Instance.new("BodyVelocity")
BodyVelocity.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
BodyVelocity.Velocity = Vector3.new(0, 0, 0)

S.RS.Heartbeat:Connect(function()
    pcall(function()
        local t = getgenv().LockedTarget
        if t and t:FindFirstChild("HumanoidRootPart") and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health > 0 then
            local hrp = LP.Character.HumanoidRootPart
            local tHrp = t.HumanoidRootPart
            
            if not hrp:FindFirstChild("BodyVelocity") then
                local newBv = BodyVelocity:Clone()
                newBv.Parent = hrp
            end

            local d = (hrp.Position - tHrp.Position).Magnitude
            local targetPos = tHrp.CFrame * CFrame.new(0, 2, 3)

            if d > 100 then
                local tween = TweenService:Create(hrp, TweenInfo.new(d/300, Enum.EasingStyle.Linear), {CFrame = targetPos})
                tween:Play()
            else
                hrp.CFrame = targetPos
                hrp.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                hrp.AssemblyAngularVelocity = Vector3.new(0, 0, 0)
            end
            
            for _, v in pairs(LP.Character:GetDescendants()) do
                if v:IsA("BasePart") and v.CanCollide then v.CanCollide = false end
            end
        else
            if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
                local activeBv = LP.Character.HumanoidRootPart:FindFirstChild("BodyVelocity")
                if activeBv then activeBv:Destroy() end
            end
        end
    end)
end)

local function preventSitting(character)
    local humanoid = character:WaitForChild("Humanoid", 5)
    if humanoid then humanoid:SetStateEnabled(Enum.HumanoidStateType.Seated, false) end
end
if LP.Character then preventSitting(LP.Character) end
LP.CharacterAdded:Connect(preventSitting)

for _, v in pairs(S.W:GetDescendants()) do
    if v:IsA("Seat") or v:IsA("VehicleSeat") then v.Disabled = true end
end
S.W.DescendantAdded:Connect(function(v)
    if v:IsA("Seat") or v:IsA("VehicleSeat") then v.Disabled = true end
end)

local function VirtualClick()
    pcall(function()
        S.V:SendMouseButtonEvent(0, 0, 0, true, game, 1)
        task.wait()
        S.V:SendMouseButtonEvent(0, 0, 0, false, game, 1)
    end)
end

t_spawn(function()
    while t_wait(0.1) do
        pcall(function()
            if getgenv().LockedTarget and LP.Character and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health > 0 then
                VirtualClick()
            end
        end)
    end
end)

t_spawn(function()
    local keys = {Enum.KeyCode.Z, Enum.KeyCode.X, Enum.KeyCode.C, Enum.KeyCode.V}
    while t_wait(0.5) do
        pcall(function()
            if getgenv().LockedTarget and LP.Character and LP.Character:FindFirstChild("Humanoid") and LP.Character.Humanoid.Health > 0 then
                local dist = (LP.Character.HumanoidRootPart.Position - getgenv().LockedTarget.HumanoidRootPart.Position).Magnitude
                if dist <= 80 then
                    if getgenv().Setting.AutoHaki then
                        S.V:SendKeyEvent(true, Enum.KeyCode.J, false, game)
                        task.wait(0.05)
                        S.V:SendKeyEvent(false, Enum.KeyCode.J, false, game)
                    end
                    if getgenv().Setting.AutoAwakening then
                        S.V:SendKeyEvent(true, Enum.KeyCode.Y, false, game)
                        task.wait(0.05)
                        S.V:SendKeyEvent(false, Enum.KeyCode.Y, false, game)
                    end
                    if getgenv().Setting.SpamDash then
                        S.V:SendKeyEvent(true, Enum.KeyCode.Q, false, game)
                        task.wait(0.05)
                        S.V:SendKeyEvent(false, Enum.KeyCode.Q, false, game)
                    end
                    if getgenv().Setting.AutoSkills then
                        for _, key in ipairs(keys) do
                            if not getgenv().LockedTarget then break end
                            S.V:SendKeyEvent(true, key, false, game)
                            task.wait(0.05)
                            S.V:SendKeyEvent(false, key, false, game)
                        end
                    end
                end
            end
        end)
    end
end)

t_spawn(function()
    while t_wait(1) do
        pcall(function()
            if getgenv().Setting.HideOthers then
                for _, player in ipairs(S.P:GetPlayers()) do
                    if player ~= LP and player.Character then
                        local isTarget = (getgenv().LockedTarget and getgenv().LockedTarget == player.Character)
                        for _, v in ipairs(player.Character:GetDescendants()) do
                            if v:IsA("BasePart") and v.Name ~= "HumanoidRootPart" then
                                v.Transparency = isTarget and (getgenv().Setting.Hitbox.Enabled and getgenv().Setting.Hitbox.Transparency or 0) or 1
                            elseif v:IsA("Decal") then
                                v.Transparency = isTarget and 0 or 1
                            elseif not isTarget and (v:IsA("Accessory") or v:IsA("Clothing") or v:IsA("Shirt") or v:IsA("Pants")) then
                                v:Destroy()
                            end
                        end
                    end
                end
            end
        end)
    end
end)

